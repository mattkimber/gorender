...
func getViewportPlane(angle float64, m manifest.Manifest, size geometry.Point) geometry.Plane {
	...
	midpoint := geometry.Vector3{X: float64(size.X-1) / 2.0, Y: float64(size.Y-1) / 2.0, Z: float64(m.Size.Z-1) / 2.0}
	viewpoint := midpoint.Add(getRenderDirection(angle, elevationAngle).MultiplyByConstant(100.0))

	planeNormalXComponent := math.Abs((float64(m.Size.X-1) / 2.0) * cos * math.Sin(degToRad(elevationAngle)))
	planeNormalYComponent := math.Abs((float64(m.Size.Y-1) / 2.0) * sin * math.Sin(degToRad(elevationAngle)))
	...
	renderNormalXComponent := math.Abs((float64(m.Size.X-1) / 2.0) * sin)
	renderNormalYComponent := math.Abs((float64(m.Size.Y-1) / 2.0) * cos)
